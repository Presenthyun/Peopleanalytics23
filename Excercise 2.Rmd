...
knitr::opts_chunk1$set(echo = TRUE)
library(tidyverse)
library(lubridate)
library(arrow)
```
...
data_path <- "/Users/brita/Downloads/app_data_sample.parquet"
...
```
#Overall distributions for gender and race
```{r exercise}
library(ggplot2)
ggplot(applications, aes(x = gender)) +
  geom_bar() +
  xlab("Gender") +
  ylab("Count") +
...
ggplot(applications, aes(x = race)) +
  geom_bar() +
  xlab("race") +
  ylab("Count") +
...
```
#Overall distributions for tenure
```{r exercise}
ggplot(data = applications)
ggplot(data = applications) +
  geom_histogram(aes(x = tenure_days))  
...
```
#Distribution of Race of tcs
```{r exercise}
ggplot(data = applications) +
  geom_bar(aes(x = tc, fill = race), position = "fill") +
  xlab("Building") +
  ylab("Proportion of Race") +
  ggtitle("Distribution of Race by Building")

ggplot(data = applications) +
  geom_bar(aes(x = tc, fill = gender), position = "fill") +
  xlab("Building") +
  ylab("Proportion of gender") +
  ggtitle("Distribution of gender by Building")
...
##Even with the help of GPT, I had difficulty in creating a script which fully represented correlations so I had to rely on shared codes with classmates. I was able to notice that there is a significantly higher frequency of the white race and male gender ("white males") in every technology center and the next largest prportion of race is asians. Black and hispanic people are not well represented within this dataset.It was also possible to notice that most workers had long tenure days and the asian race seemed to account for longer tenure days in general